...
iWORK.Major.bindEditLink = function() {
	$('[data-itsp-edit-link]').click(function(event) {
		...
		location.href = '/iWORK/Major/Curriculum.do?seq='+seq;
	});
};
	...
	var doSave = function() {
		...
		itsp.ajax.doPostFormJSON(form, function(data) {
			if (data.header.code == itsp.ajax.CODE_SUCCESS) {
				location.href = '/iWORK/Major/CurriculumList.do?aboutSeq='+$('#fm input[name=aboutSeq]').val();
			} else {
...
iWORK.Major.bindDeleteLink = function() {
	$('[data-itsp-delete-link]').click(function(event) {
		...
		itsp.ajax.doPostJSON('/iWORK/Major/Curriculum/deleteProcData', { seq : seq },  function(data) {
			if (data.header.code == itsp.ajax.CODE_SUCCESS) {
				location.reload();
			} else {
				alert(data.header.message);
...
iWORK.Major.bindOrderNumLink = function() {
	$('[data-itsp-orderNum-link]').click(function(event) {
		...
		itsp.ajax.doPostJSON('/iWORK/Major/Curriculum/orderNumProc', { data : dataArray }, function(data) {

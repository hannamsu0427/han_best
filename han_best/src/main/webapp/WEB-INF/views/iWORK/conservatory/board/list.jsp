					...
					<div class="post_tbl">
						...
						<div class="post_list">
							<table class="post_head">
								...
								<colgroup>
									<col width="7%">
									<col width="${ 'Y' eq dataConfig.replyYn || 'qna' eq dataConfig.type ? '50':'57'}%">
									<col width="10%">
									<col width="12%">
							...
							<c:if test="${not empty noticeList}">
								<ul class="noti_area">
									<c:forEach items="${noticeList }" var="notice">
										...
										<li class="post_tit">
											<a href="#" data-itsp-view-link="<c:out value="${notice.seq}"/>">
												<div class="subject <c:if test="${today < regDate }">new</c:if>"><span class="txt_head">[공지]</span>
							...
							<c:if test="${not empty dataList}">
								<ul class="list_area">
									<c:forEach items="${dataList }" var="data">
										...
										<fmt:parseNumber integerOnly="true" type="number" value="${tempRegDate}" var="regDate" />
										<li class="post_tit">
											<a href="#" data-itsp-view-link="<c:out value="${data.seq}"/>">
												<div class="subject <c:if test="${today < regDate }">new</c:if>"><div class="num"><c:out value="${totalCount - data.rnum + 1}"/></div>
													<span class="subject_click">
